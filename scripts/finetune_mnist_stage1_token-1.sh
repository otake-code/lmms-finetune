...
for BS in "${BATCH_SIZES[@]}"; do
  for LR in "${LEARNING_RATES[@]}"; do
    for EP in "${EPOCHS[@]}"; do
      ...
      RUN_ID="token-1_gbs${GLOBAL_BS}_lr${LR}_ep${EP}_mnist_$DATE"

      torchrun $DISTRIBUTED_ARGS train_add_head_token-1.py \
        --model_id $MODEL_ID \
        --model_local_path $MODEL_LOCAL_PATH \
        --data_path $TRAIN_DATA_PATH \
        --image_folder $IMAGE_FOLDER \
        --output_dir ./checkpoints/mnist/token_-1/${RUN_ID} \
        --run_name $RUN_ID \
